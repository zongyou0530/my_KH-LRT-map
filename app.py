...
<style>
    ...
    /* 紅點波紋 CSS */
    .current-pos-wrapper {{ display: flex; justify-content: center; align-items: center; width: 40px; height: 40px; }}
    .current-pos-dot {{ background: #ff5252; border-radius: 50%; width: 14px; height: 14px; border: 2px solid white; z-index: 1000; position: relative; }}
    .current-pos-pulse {{ position: absolute; width: 14px; height: 14px; background: rgba(255, 82, 82, 0.6); border-radius: 50%; animation: pulse-out 2s infinite ease-out; }}
    @keyframes pulse-out {{ 0% {{ transform: scale(1); opacity: 1; }} 100% {{ transform: scale(4); opacity: 0; }} }}

    .board-container {{ background-color: #1a1d23; border: 1px solid #30363d; border-radius: 12px; overflow: hidden; margin-bottom: 10px; min-height: 100px; }}
...
with col_map:
    ...
    if u_pos:
        folium.Marker(u_pos, icon=folium.DivIcon(html='<div class="current-pos-wrapper"><div class="current-pos-pulse"></div><div class="current-pos-dot"></div></div>')).add_to(m)
    if token:
        try:
...
with col_info:
    ...
    if u_pos:
        ...
        st.markdown(f'<div class="status-text" style="color:#ff5252;">⚠️ 未取得 GPS 定位 (請點擊瀏覽器允許)</div>', unsafe_allow_html=True)
...
with col_msg:
    st.markdown(f'''<div class="board-container">
                ...
                <div style="padding:15px; font-family:MyHand; font-size: 18px;">
                資料由 TDX 提供，請勿長時間開啟，點數會不夠。
                </div>
                </div>''', unsafe_allow_html=True)

with col_log:
    st.markdown(f"""<div class="board-container">
                <div class="board-header">📦 系統更新紀錄 (v1.2.4)</div>
                <div style="padding:15px; color:#cbd5e0; font-size:11px;">
                • 內容補全：重新找回遺失的「作者留言」區塊。<br>
                • 標題強制換行：高雄輕軌 / 即時位置地圖。<br>
                • 圖標雙色：綠色(順行)與藍色(逆行)車頭。<br>
                • 紅點波紋：強化 DivIcon 擴散動畫效果。
                </div>
                </div>""", unsafe_allow_html=True)
