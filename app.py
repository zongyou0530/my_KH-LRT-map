...
<style>
    ...
    .info-box { 
        ...
        padding: 15px; border-radius: 10px; margin-bottom: 20px; color: #0d47a1; 
    }
    /* 抵達時間卡片 */
    .arrival-card { 
    ...
    .dest-text { color: #666; font-size: 0.9em; margin-bottom: 5px; }
    .time-normal { font-size: 1.6em; color: #4D0000; }
    .time-urgent { font-size: 1.6em; color: #FF0000; }
</style>
...
st.markdown('<div class="info-box">💡 <b>系統提示：</b> 輕軌預計抵達時間 (不分順逆，依序顯示)。</div>', unsafe_allow_html=True)
...
with col2:
    ...
    sel_st_label = st.selectbox("手機端請撥動選擇：", list(STATION_MAP.keys()), index=19)
            ...
            if matched:
                ...
                for item in matched:
                    ...
                    <div class="arrival-card">
                        <div class="dest-text">往：{dest}</div>
