...
<style>
    ...
    /* 抵達時間顏色規範：#4D0000(一般), #FF0000(即將抵達), 且均不加粗 */
...
# 重新顯示藍色對話框
st.markdown('<div class="info-box">💡 <b>系統提示：</b> 已優化順逆雙向資料抓取，抵達時間為 2 分鐘內將以鮮紅色標示，其餘為深褐色。</div>', unsafe_allow_html=True)
...
with col2:
    ...
    sel_st_full = st.selectbox("請選擇車站（手機端可撥動）：", LRT_STATIONS, index=19) 
    ...
    if token:
        try:
            ...
            if resp.status_code == 200:
                ...
                matched_list = [entry for entry in all_raw_data if search_target in entry.get('StationName', {}).get('Zh_tw', '') and entry.get('EstimateTime') is not None]
            ...
            else: st.warning("📡 伺服器忙碌中")
        except: board_time = "連線重試中..."

st.markdown(f'<div style="color:gray; font-size:0.8em; margin-top:20px;">📍 最後同步：{map_time} (每 30 秒自動更新)</div>', unsafe_allow_html=True)
