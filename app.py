...
# 藍色對話框
st.markdown('<div class="info-box">💡 <b>終極修正：</b> 採用車站代碼 (StationID) 進行絕對比對，解決順行資料消失問題。</div>', unsafe_allow_html=True)

# 圖標說明 (Legend) 回歸
st.markdown('<div class="legend-box">📍 <b>即時圖例：</b> <span style="color:#2e7d32;">● 順行 (外圈)</span> | <span style="color:#1565c0;">● 逆行 (內圈)</span> | 🚥 綠色軌道：全線路網</div>', unsafe_allow_html=True)
...
with col2:
    ...
    target_id = STATION_MAP[sel_st_label] # 取得如 "C20"
    ...
    if token:
        try:
            ...
            # 使用 StationID 絕對比對，這能避開名稱內含空格或編碼不同的問題
